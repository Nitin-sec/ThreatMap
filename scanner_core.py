...
class ScannerKit:
    ...
    def run_nmap(target: Target, mode: str = MODE_BALANCED) -> list[dict]:
        ...
        if mode == MODE_AGGRESSIVE:
            cmd = ["nmap", "-T4 --top-ports 100", "-sC", "-A", "-Pn", "-p-",
                   "--min-rate", "2000", "--max-retries", "1",
                   "-T4", "-oX", out, target.domain]
            fallback = ["nmap", "-T4 --top-ports 100", "-sC", "-Pn", "--top-ports", "5000",
                        "-T4", "-oX", out, target.domain]
            timeout = 900
        else:
            cmd = ["nmap", "-T4 --top-ports 100", "-sC", "-Pn", "--top-ports", "1000",
                   "-T4", "--max-retries", "1", "-oX", out, target.domain]
            fallback = ["nmap", "-T4", "-Pn", "--top-ports", "50", "-oX", out, target.domain]
            timeout = 300
